...
namespace ProductModel.Helpers
{
    public class Mapping
    {
        ...
        public static Order ToOrderViewModel(OrderDB order)
        {
            return new Order()
            {
                Id = order.Id,
                Cart = CardDBToCart(order.Cart),
                User = order.User
            };
        }
        public static List<Order> ToOrdersViewModel(List<OrderDB> orders)
        {
            var result = new List<Order>();
            foreach(var order in orders)
            {
                result.Add(ToOrderViewModel(order));
            }
            return result;
        }
    }
